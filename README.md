...
- [Show latest migration details](#show-latest-migration-details)
- [Downgrade one migration](#downgrade-one-migration)
- [Reset all migrations (development only!)](#reset-all-migrations-development-only)
  ...
  - [26) Notification System](#26-notification-system-1)
...
- **make run**: Start the dev server with reload using your local venv (development mode).
- **make run-dev**: Same as `make run` - development mode with uvicorn and auto-reload.
- **make run-prod**: Start the server in production mode using gunicorn + uvicorn workers.
  - Example: `make run` then open <http://localhost:8000>

### Python virtualenv

- **make venv-init**: Create `venv/` and install base tooling (uv, pip-tools, wheel).
  - Example: `make venv-init`
