...
class ArticleService:
    async def create_article(self, db: AsyncSession, payload: ArticleCreate):
        return await article_repository.create(
            db, title=payload.title, content=payload.content
        )

    async def get_article(self, db: AsyncSession, article_id: int):
        return await article_repository.get(db, article_id)

    async def list_articles(self, db: AsyncSession, skip: int = 0, limit: int = 100):
