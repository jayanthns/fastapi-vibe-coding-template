...
class TestArticleAPIs:
    ...
    This class contains all article API tests organized by functionality:
    - GET operations (retrieve articles)
    - POST operations (create articles)
    - PATCH operations (update articles)
    - DELETE operations (delete articles)
    ...
    def setup_method(self):
        ...
        Similar to unittest.TestCase.setUp()
        """
        # Initialize test data that might be needed across tests
        ...
        self.sample_update_data = {
            ...
            "content": "This is an updated sample article for testing purposes.",
        }
        ...
        self.invalid_article_data = [
            ...
            {"title": "A" * 300, "content": "Title too long"},
        ]
    ...
    def teardown_method(self):
        ...
        Similar to unittest.TestCase.tearDown()
        """
        # Clean up any test data if needed
        # Note: The database is already cleaned up by the conftest.py fixtures
        # This method is here for any additional cleanup if needed in the future
        self.created_article_ids.clear()

    def setup_class(cls):
        """
        Set up test fixtures before the entire test class.

        This method is called once before any test methods in the class.
        Similar to unittest.TestCase.setUpClass()
        """
        # This can be used for expensive setup that only needs to happen once
        # For example, creating test users, setting up external services, etc.
        pass

    def teardown_class(cls):
        """
        Clean up after the entire test class.

        This method is called once after all test methods in the class.
        Similar to unittest.TestCase.tearDownClass()
        """
        # This can be used for cleanup that only needs to happen once
        # For example, cleaning up external services, removing test data, etc.
        pass
    ...
    def create_test_article_data(
        self, title_suffix: str = "", content_suffix: str = ""
    ):
    ...
    async def test_create_article_success(
        ...
        article_data = self.sample_article_data.copy()
    ...
    async def test_create_article_missing_title(
        ...
        article_data = self.invalid_article_data[0]  # Missing title
    ...
    async def test_create_article_missing_content(
        ...
        article_data = self.invalid_article_data[1]  # Missing content
    ...
    async def test_create_article_empty_title(
        ...
        article_data = self.invalid_article_data[2]  # Empty title
    ...
    async def test_create_article_long_title(
        ...
        article_data = self.invalid_article_data[3]  # Title too long
    ...
    async def test_get_articles_list_empty(
        self, async_session: AsyncSession, client: TestClient
    ):
    ...
    async def test_update_article_success(
        ...
        # Create a test article in the database
        article = Article(**self.sample_article_data)
        async_session.add(article)
        await async_session.commit()
        await async_session.refresh(article)
    ...
    async def test_article_with_very_long_content(
        ...
        long_content = (
            "This is a very long article content. " * 1000
        )  # ~40KB of content
    ...
    async def test_article_with_whitespace(
        self, async_session: AsyncSession, client: TestClient
    ):
    ...
    async def test_article_crud_workflow(
        ...
        # 1. Create article
        create_data = self.create_test_article_data(
            "CRUD", "for full CRUD workflow testing"
        )
        ...
        self.track_created_article(article_id)
        ...
        # 3. Update article
        update_data = self.create_test_article_data(
            "Updated CRUD", "updated through the CRUD workflow"
        )

        update_response = client.patch(
            f"/api/v1/articles/{article_id}", json=update_data
        )
