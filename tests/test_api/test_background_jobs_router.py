...
class TestBackgroundJobsRouter:
    async def test_list_jobs_endpoint(self, async_client, async_session):
        ...
        job1 = await repo.create(
            task_name="task_1", message_id="msg_1", status=JobStatus.COMPLETED
        )
        job2 = await repo.create(
            task_name="task_2", message_id="msg_2", status=JobStatus.FAILED
        )
        ...
        response = await async_client.get(
            "/api/v1/jobs/", params={"status": "COMPLETED"}
        )
        assert response.status_code == 200
        data = response.json()["data"]
        ...
        response = await async_client.get(
            "/api/v1/jobs/", params={"task_name": "task_2"}
        )
        assert response.status_code == 200
        data = response.json()["data"]
